...
class CourseCreateForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validation = CourseValidator(course_id, 
            display_name, course_creator_username)
...
class CourseAddUserForm(forms.Form):
    ...
    def clean_user_resource_uri(self):
        ...
        if not self._user:
            raise forms.ValidationError('Invalid user_resource_uri!')
    ...
    def clean(self):
        ...
        if group is None:
            msg = '%s group for %s does not exist! Contact server admin!' % (
            ...
            msg = 'User is already a %s.' % cleaned_data.get(
                'staff_or_instructor')
            self._errors['user_resource_uri'] = self.error_class([msg])
            if 'user_resource_uri' in cleaned_data:
                del cleaned_data['user_resource_uri']
